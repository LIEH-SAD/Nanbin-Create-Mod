...
public final class JSDrawContext {
	...
	public JSDrawContext(MatrixStack matrices, VertexConsumerProvider vertexConsumers, Direction facing, float startX, float startY, float cellSize, int cellCount, boolean transparentBlack, int backgroundColor) {
	...
	public void drawText(String text, double x, double y, double w, double h, double color, double scale, boolean bold, boolean centered) {
		RenderCRTRailwaySign.renderJSTextInBox(text, matrices, vertexConsumers, facing, startX + (float) x, startY + (float) y, (float) w, (float) h, normalizeColor(toArgb(color)), (float) scale, bold, centered);
	}

	public void drawTexture(String identifier, double x, double y, double w, double h, double color) {
		RenderCRTRailwaySign.renderJSTexture(new Identifier(identifier), matrices, vertexConsumers, facing, startX + (float) x, startY + (float) y, (float) w, (float) h, toArgb(color));
	}

	public void drawRect(double x, double y, double w, double h, double color) {
		RenderCRTRailwaySign.renderJSRect(matrices, vertexConsumers, facing, startX + (float) x, startY + (float) y, (float) w, (float) h, normalizeColor(toArgb(color)));
	}

	public void drawLine(double x1, double y1, double x2, double y2, double thickness, double color) {
		RenderCRTRailwaySign.renderJSLine(matrices, vertexConsumers, facing, startX + (float) x1, startY + (float) y1, startX + (float) x2, startY + (float) y2, (float) thickness, normalizeColor(toArgb(color)));
	}
