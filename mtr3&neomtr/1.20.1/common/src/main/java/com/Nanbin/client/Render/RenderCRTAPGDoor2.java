...
/**
 * CRT 第二代矮屏蔽门渲染器：绘制门页（随开合平移）、门顶指示灯（关门/列车接近时闪烁，
 * 全开常绿）以及站台面地灯（站台为 CRT 站台时）。
 */
public class RenderCRTAPGDoor2 implements BlockEntityRenderer<BlockCRTAPGDoor2.BlockEntity> {
	...
	public RenderCRTAPGDoor2(BlockEntityRendererFactory.Context context) {
	}

	@Override
	public void render(BlockCRTAPGDoor2.BlockEntity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
		final World world = entity.getWorld();
		if (world == null) {
			return;
		}
		final BlockPos blockPos = entity.getPos();
		if (!(world.getBlockState(blockPos).getBlock() instanceof BlockCRTAPGDoor2)) {
			return;
		}
		...
		// 门顶指示灯：贴在门页上方的玻璃旁（穷举实例，两个 MTR 原生门也一并识别）
		if (half) {
			final Block neighbour = world.getBlockState(blockPos.offset(side ? facing.rotateYClockwise() : facing.rotateYCounterclockwise())).getBlock();
			if (neighbour instanceof BlockCRTAPGGlass2 || neighbour instanceof BlockCRTAPGGlassEnd2 || neighbour instanceof BlockAPGGlass || neighbour instanceof BlockAPGGlassEnd) {
				...
				if (isDoorFullyOpen) {
					...
					lightTexture = gameTime % (long) ARRIVAL_WARNING_BLINK_PERIOD < ARRIVAL_WARNING_BLINK_PERIOD / 2.0 ? "off" : "yellow";
					translucent = true;
				} else {
					lightTexture = "off";
					translucent = false;
				}
				drawModel(matrices, vertexConsumers, baseMatrix, blockTexture("crt_apg_door_light_" + lightTexture + ".png"), CRT_APG_LIGHT_2, translucent, light, overlay, graphics -> {
					graphics.translate(side ? -0.5F : 0.5F, 0.0F, 0.0F);
					graphics.scale(0.5F, 1.0F, 1.0F);
				});
			}
		}

		// 站台面地灯：仅当门下方为 CRT 站台方块时绘制（左右两侧用不同贴图避免镜像问题）
		if (!half) {
			final Block blockBelow = world.getBlockState(blockPos.down()).getBlock();
			if (blockBelow instanceof BlockCRTPlatform) {
				final String groundTexture = side ? "crt_ground_light_off.png" : "crt_ground_light_off_flipped.png";
				drawModel(matrices, vertexConsumers, groundMatrix, blockTexture(groundTexture), CRT_APG_GROUND_LIGHT_2, false, light, overlay, null);
			}
		}
		...
		drawModel(matrices, vertexConsumers, doorMatrix, blockTexture("crt_apg_door_2_" + (half ? "top" : "bottom") + "_" + (side ? "right" : "left") + ".png"), half ? CRT_APG_TOP_2 : CRT_APG_BOTTOM_2, false, light, overlay, null);

		if (half && !unlocked) {
			drawModel(matrices, vertexConsumers, doorMatrix, new Identifier("mtr", "textures/block/sign/door_not_in_use.png"), CRT_APG_DOOR_LOCKED_2, false, light, overlay, null);
		}
	}

	@Override
	public boolean rendersOutsideBoundingBox(BlockCRTAPGDoor2.BlockEntity entity) {
		return true;
	}
	...
	private boolean isTrainApproaching(BlockPos doorPos) {
		try {
			...
			Init.LOGGER.error("[CRTAPGDoor2] isTrainApproaching error", e);
		}
		return false;
	}
	...
	private static class ModelAPGDoorLight extends Model {
		...
		private ModelAPGDoorLight() {
			...
			bone.texOffs(0, 4).addBox(-0.5F, -9.0F, -7.0F, 1, 1, 3, 0.05F, false);
	...
	private static class ModelAPGDoorGround extends Model {
		...
		private ModelAPGDoorGround() {
			...
			bone = new ModelMapper(modelDataWrapper);
			bone.texOffs(0, 0).addBox(-8.0F, 0.0F, -8.0F, 16, 1, 16, 0.0F, false);
	...
	private static class ModelAPGDoorBottom extends Model {
		...
		private ModelAPGDoorBottom() {
			...
			bone = new ModelMapper(modelDataWrapper);
			bone.texOffs(0, 0).addBox(-8.0F, -16.0F, -7.0F, 16, 16, 1, 0.0F, false);
			bone.texOffs(0, 17).addBox(-8.0F, -6.0F, -8.0F, 16, 6, 1, 0.0F, false);
			final ModelMapper cube = new ModelMapper(modelDataWrapper);
			cube.setPos(0.0F, -6.0F, -8.0F);
			bone.addChild(cube);
			cube.setRotationAngle(-0.7854F, 0.0F, 0.0F);
			cube.texOffs(0, 24).addBox(-8.0F, -2.0F, 0.0F, 16, 2, 1, 0.0F, false);
