...
public class RoadNameScreen extends Screen {
	...
	protected void init() {
		...
		final ButtonWidget saveButton = ButtonWidget.builder(Text.translatable("gui.nanbin.road_name_save"), button -> {
			...
			this.close();
		}).dimensions(fieldX, fieldY + 6, fieldWidth, TEXT_FIELD_HEIGHT).build();
		...
		final ButtonWidget undoButton = ButtonWidget.builder(Text.translatable("gui.nanbin.undo"), button -> {
			final String[] texts = getTexts();
			RoadNameClipboard.undo(blockPos, texts);
			setTexts(texts);
			button.active = RoadNameClipboard.canUndo(blockPos);
		}).dimensions(buttonX, buttonY + 90, 30, 20).build();
		undoButton.active = RoadNameClipboard.canUndo(blockPos);

		final ButtonWidget copyButton = ButtonWidget.builder(Text.translatable("gui.nanbin.copy"), button -> RoadNameClipboard.copy(getTexts())).dimensions(buttonX, buttonY, 30, 20).build();

		final ButtonWidget pasteButton = ButtonWidget.builder(Text.translatable("gui.nanbin.paste"), button -> {
			final String[] texts = getTexts();
			RoadNameClipboard.paste(blockPos, texts);
			setTexts(texts);
			undoButton.active = RoadNameClipboard.canUndo(blockPos);
		}).dimensions(buttonX, buttonY + 30, 30, 20).build();
		pasteButton.active = RoadNameClipboard.canPaste();

		final ButtonWidget clearButton = ButtonWidget.builder(Text.translatable("gui.nanbin.clear"), button -> MinecraftClient.getInstance().setScreen(new RailwaySignClearConfirmScreen(this, Text.translatable("gui.nanbin.clear.question"), () -> {
			final String[] texts = getTexts();
			RoadNameClipboard.clear(blockPos, texts);
			setTexts(texts);
			undoButton.active = RoadNameClipboard.canUndo(blockPos);
		}))).dimensions(buttonX, buttonY + 60, 30, 20).build();
	...
	public void render(DrawContext context, int mouseX, int mouseY, float delta) {
		try {
			...
			context.drawCenteredTextWithShadow(this.textRenderer, Text.translatable("gui.nanbin.road_name"), this.width / 2, 8, ARGB_WHITE);

			drawTextureAndDisplays(context.getMatrices());

			int fieldY = 40;
			for (int i = 0; i < textFields.length; i++) {
				context.drawTextWithShadow(this.textRenderer, Text.translatable("gui.nanbin.road_name_" + (i + 1)), fieldX, fieldY - LABEL_HEIGHT, ARGB_WHITE);
				fieldY += ROW_SPACING;
			}
