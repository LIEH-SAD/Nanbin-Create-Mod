...
public class TicketMachineScreen extends ScreenMapper {
	...
	public TicketMachineScreen(int balance) {
		...
		for (int i = 0; i < BUTTON_COUNT; ++i) {
			final int index = i;
			this.buttons[i] = new ButtonWidget(0, 0, 0, BUTTON_HEIGHT, Text.literal("$" + PacketHandler.ADD_AMOUNTS[i]), (button) -> {
	...
	protected void init() {
		...
		for (int i = 0; i < BUTTON_COUNT; ++i) {
			...
			button.x = getButtonX(i);
			button.y = getButtonY(i);
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		this.textRenderer.draw(matrices, this.balanceText, (int) (windowX + 241 * RELATIVE_X - this.textRenderer.getWidth(emeraldsText) - 6 - this.textRenderer.getWidth(this.balanceText)), (int) (windowY + 8 * RELATIVE_X), COLOR_TEXT);
		this.textRenderer.draw(matrices, emeraldsText, (int) (windowX + 241 * RELATIVE_X - this.textRenderer.getWidth(emeraldsText)), (int) (windowY + 8 * RELATIVE_X), COLOR_TEXT);
		this.textRenderer.draw(matrices, stationText, (int) (windowX + 9 * RELATIVE_X), (int) (windowY + 8 * RELATIVE_X), COLOR_TEXT);

		for (int i = 0; i < BUTTON_COUNT; ++i) {
			final Text emeraldCostText = Text.translatable("gui.nanbin.emerald", PacketHandler.EMERALD_COSTS[i]);
			this.textRenderer.draw(matrices, emeraldCostText, getButtonX(i) + (BUTTON_WIDTH - this.textRenderer.getWidth(emeraldCostText)) / 2, getButtonY(i) + BUTTON_HEIGHT + 2, -1);
		}
