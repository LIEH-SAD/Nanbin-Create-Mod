...
public class BusTicketProcessorScreen extends ScreenMapper {
	...
	protected void init() {
		...
		modeButton = new ButtonWidget(guiLeft + MARGIN, guiTop + 34, componentWidth, BUTTON_HEIGHT, Text.translatable(MODE_NAMES[mode]), button -> {
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		drawCenteredText(matrices, textRenderer, title, width / 2, guiTop + 12, COLOR_TITLE);

		if (mode != BlockBusTicketProcessor.MODE_FIXED_AMOUNT) {
			drawTextWithShadow(matrices, textRenderer, Text.translatable("gui.nanbin.ticket_processer.two_tap.disabled"), guiLeft + MARGIN, guiTop + 88, COLOR_DISABLED_HINT);
		}
