...
public class BlockGreenFence extends Block {
    ...
    public BlockState getPlacementState(ItemPlacementContext context) {
        ...
        if (canPlace) {
            ...
            world.setBlockState(clickPos.up(3), Blocks.GREENFENCE_TOP.get().getDefaultState().with(FACING, facing), 3);
        ...
        while (true) {
            if (world.getBlockState(current).getBlock() instanceof BlockGreenFence || world.getBlockState(current).getBlock() instanceof BlockGreenFenceTop) {
                ...
                if (!player.isCreative()) {
                    dropStack(world, current, new net.minecraft.item.ItemStack(Blocks.GREENFENCE.get()));
                }
            }
