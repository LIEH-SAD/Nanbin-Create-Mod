...
public class BlockGreenFence extends Block {
    ...
    public BlockState getPlacementState(ItemPlacementContext context) {
        ...
        if (canPlace) {
            ...
            world.setBlockState(clickPos.up(3), Blocks.GREENFENCE_TOP.getDefaultState().with(FACING, facing), 3);
    ...
    public void onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
        ...
        while (true) {
            ...
            if (world.getBlockState(check).getBlock() instanceof BlockGreenFence || world.getBlockState(check).getBlock() instanceof BlockGreenFenceTop) {
                bottomPos = check;
            } else break;
        }
        ...
        while (true) {
            if (world.getBlockState(current).getBlock() instanceof BlockGreenFence || world.getBlockState(current).getBlock() instanceof BlockGreenFenceTop) {
                world.setBlockState(current, net.minecraft.block.Blocks.AIR.getDefaultState(), 3);
                if (!player.isCreative()) {
                    dropStack(world, current, new net.minecraft.item.ItemStack(Blocks.GREENFENCE));
                }
            }
