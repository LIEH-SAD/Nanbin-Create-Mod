...
import static com.Nanbin.Blocks.Blocks.BLUEFENCE_TOP;
...
public class BlockBlueFence extends Block {
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack stack) {
        ...
        BlockPos bottom = pos;
        while (world.getBlockState(bottom.down()).getBlock() instanceof BlockBlueFence ||
                world.getBlockState(bottom.down()).getBlock() instanceof BlockBlueFenceTop) {
            bottom = bottom.down();
        }
        ...
        for (int i = 0; i <= 3; i++) {
            ...
            if (i < 3) {
                ...
                targetState = BLUEFENCE_TOP.get().data.getDefaultState().with(BlockBlueFenceTop.FACING, facing);
            }

            BlockState existingState = world.getBlockState(targetPos);
            if (existingState.isAir() ||
                    existingState.getBlock() instanceof BlockBlueFence ||
                    existingState.getBlock() instanceof BlockBlueFenceTop) {
