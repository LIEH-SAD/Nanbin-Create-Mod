...
import static com.Nanbin.Blocks.Blocks.METALFENCE_TOP;
...
public class BlockMetalFence extends Block {
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack stack) {
        ...
        BlockPos bottom = pos;
        while (world.getBlockState(bottom.down()).getBlock() instanceof BlockMetalFence ||
                world.getBlockState(bottom.down()).getBlock() instanceof BlockMetalFenceTop) {
            bottom = bottom.down();
        }
        ...
        for (int i = 0; i <= 3; i++) {
            ...
            if (i < 3) {
                ...
                targetState = METALFENCE_TOP.get().data.getDefaultState().with(BlockMetalFenceTop.FACING, facing);
            }

            BlockState existingState = world.getBlockState(targetPos);
            if (existingState.isAir() ||
                    existingState.getBlock() instanceof BlockMetalFence ||
                    existingState.getBlock() instanceof BlockMetalFenceTop) {
