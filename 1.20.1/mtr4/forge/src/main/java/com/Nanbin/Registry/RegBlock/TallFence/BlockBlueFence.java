...
public class BlockBlueFence extends BlockExtension {
    ...
    public BlockBlueFence(BlockSettings settings) {
        super(settings);
    }
    ...
    private boolean isFenceBlock(Block block) {
        return block.data instanceof BlockBlueFence || block.data instanceof BlockBlueFenceTop;
    }
    ...
    private BlockState createFenceState(Direction facing, int heightIndex) {
        if (heightIndex < BOTTOM_SECTION_COUNT) {
            ...
            Block topBlock = BLUEFENCE_TOP.get();
