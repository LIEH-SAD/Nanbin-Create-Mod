...
import static com.Nanbin.mappingForge.VoxelShapes.union;

public class BlockCRTTicketBarrier3 extends BlockExtension implements DirectionHelper {
    ...
    public void onEntityCollision2(BlockState state, World world, BlockPos blockPos, Entity entity) {
        if (!world.isClient() && PlayerEntity.isInstance(entity)) {
            ...
            EnumTicketBarrierOpen open = (EnumTicketBarrierOpen)IBlock.getStatePropertySafe(state, new Property((net.minecraft.world.level.block.state.properties.Property)OPEN.data));
    ...
    public void scheduledTick2(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        world.setBlockState(pos, state.with(new Property((net.minecraft.world.level.block.state.properties.Property)OPEN.data), EnumTicketBarrierOpen.CLOSED));
    }

    public BlockState getPlacementState2(ItemPlacementContext ctx) {
        return this.getDefaultState2().with(new Property((net.minecraft.world.level.block.state.properties.Property)FACING.data), ctx.getPlayerFacing().data).with(new Property((net.minecraft.world.level.block.state.properties.Property)OPEN.data), EnumTicketBarrierOpen.CLOSED);
    }

    @Nonnull
    public VoxelShape getOutlineShape2(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        Direction facing = IBlock.getStatePropertySafe(state, FACING);
        return union(IBlock.getVoxelShapeByDirection(12, 0, -7, 16, 19, 23, facing), IBlock.getVoxelShapeByDirection(-4, 0, -7, 0, 19, 23, facing));
    }
