...
import com.Nanbin.mappingForge.WorldHelper;
...
public class BlockMetalFence extends BlockExtension {
    ...
    public BlockMetalFence(BlockSettings settings) {
        super(settings);
    }
    ...
    private boolean isFenceBlock(Block block) {
        return block.data instanceof BlockMetalFence || block.data instanceof BlockBlueFenceTop;
    }
    ...
    private BlockState createFenceState(Direction facing, int heightIndex) {
        if (heightIndex < BOTTOM_SECTION_COUNT) {
            ...
            Block topBlock = METALFENCE_TOP.get();
