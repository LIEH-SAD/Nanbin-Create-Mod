...
public class BlockCRTTicketBarrier1 extends BlockDirectionalMapper {
    ...
    public BlockCRTTicketBarrier1(boolean isEntrance) {
        super(Settings.create().mapColor(MapColor.GRAY).requiresTool().strength(2.0F).luminance((state) -> 5).nonOpaque());
        ...
        if (!world.isClient && entity instanceof PlayerEntity) {
            ...
            if (open.isOpen() && playerPosRotated.z > (double)0.0F) {
                ...
                if (TicketFault.isFault(state)) {
                    ...
                    TicketSystem.EnumTicketBarrierOpen newOpen = TicketSystem.passThrough(world, pos, (PlayerEntity)entity, this.isEntrance, !this.isEntrance, SoundEvents.CRT_TICKET.get(), SoundEvents.CRT_TICKET.get(), SoundEvents.CRT_TICKET.get(), SoundEvents.CRT_TICKET.get(), (SoundEvent)null, false);
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        return (BlockState)((BlockState)this.getDefaultState().with(FACING, ctx.getHorizontalPlayerFacing())).with(OPEN, EnumTicketBarrierOpen.CLOSED);
    }
    ...
    public VoxelShape getCollisionShape(BlockState state, BlockView blockGetter, BlockPos blockPos, ShapeContext collisionContext) {
        ...
        return open.isOpen() ? base : union(
                ...
                IBlock.getVoxelShapeByDirection(0, 0, 6, 12, 24, 8, facing));
    }
