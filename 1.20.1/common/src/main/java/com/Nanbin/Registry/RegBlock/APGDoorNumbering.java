 ...
 * 该工具同时被方块 onUse（切换排序方向）与客户端渲染器使用。
 */
public final class APGDoorNumbering {
	...
	public static boolean isCRTAPGDoor(BlockState state) {
		return state.getBlock() instanceof BlockCRTAPGDoor1 || state.getBlock() instanceof BlockCRTAPGDoor2;
	}

	/** 该方块是否属于屏蔽门墙体的 CRT 部件（门或玻璃），用于沿墙连续扫描。 */
	public static boolean isCRTAPGWallBlock(BlockState state) {
		return isCRTAPGDoor(state)
				|| state.getBlock() instanceof BlockCRTAPGGlass1
				|| state.getBlock() instanceof BlockCRTAPGGlass2
				|| state.getBlock() instanceof BlockCRTAPGGlassEnd1
				|| state.getBlock() instanceof BlockCRTAPGGlassEnd2;
	}

	/** 返回门的下半部分所在位置（传入上半或下半均可）。 */
	public static BlockPos getLowerPos(World world, BlockPos pos) {
		final BlockState state = world.getBlockState(pos);
		if (isCRTAPGDoor(state) && IBlock.getStatePropertySafe(state, BlockPSDAPGDoorBase.HALF) == DoubleBlockHalf.UPPER) {
			return pos.down();
		}
		return pos;
	}
	...
	public static List<BlockPos> findRowUnitStarts(World world, BlockPos doorPos) {
		...
		for (int i = 1; i <= MAX_WALK; i++) {
			if (!collectWallStart(starts, world, unitStart.offset(axis, -i), facing)) {
				break;
			}
		}
		for (int i = 1; i <= MAX_WALK; i++) {
