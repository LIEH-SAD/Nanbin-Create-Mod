 ...
 */
public class RenderCRTStationName2 implements BlockEntityRenderer<BlockCRTStationName2.BlockEntity> {
	...
	public RenderCRTStationName2(BlockEntityRendererFactory.Context context) {
		CustomFontTextureCache.instance.selectedFont = FONT_TYPE;
		CustomFontTextureCache.instance.fontSize = FONT_SIZE;
	}

	@Override
	public void render(BlockCRTStationName2.BlockEntity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
		...
		if (!(state.getBlock() instanceof BlockCRTStationName2)) {
			return;
		}
		final Direction facing = state.get(BlockCRTStationName2.FACING);
		...
		final BlockCRTStationName2.BlockEntity.ResolvedRouteData resolved = entity.getResolvedData(stationColor);
	...
	@Override
	public boolean rendersOutsideBoundingBox(BlockCRTStationName2.BlockEntity entity) {
		return true;
	}
}