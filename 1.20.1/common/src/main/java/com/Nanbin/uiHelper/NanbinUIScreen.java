...
public class NanbinUIScreen extends Screen {
	...
	public void render(DrawContext context, int mouseX, int mouseY, float delta) {
		...
		context.fill(panelX, panelY, panelX + panelWidth, panelY + panelHeight, getPanelBackgroundColor());
		drawHeader(context);
		...
		for (final NanbinUIComponent component : components) {
			component.render(context, textRenderer, contentMouseX, contentMouseY, delta);
		}
		scrollBar.render(context, contentHeight, getViewHeight(), scroll);
	...
	private void drawHeader(DrawContext context) {
		...
		final int textX = panelX + (panelWidth - textRenderer.getWidth(label)) / 2;
		context.drawTextWithShadow(textRenderer, label, textX, panelY + (HEADER_HEIGHT - 8) / 2, HEADER_TEXT_COLOR);
	}
