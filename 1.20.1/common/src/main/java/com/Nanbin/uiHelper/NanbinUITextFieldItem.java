...
public class NanbinUITextFieldItem extends NanbinUIComponent {
	...
	protected void onLayout() {
		...
		textField.setY(y + (HEIGHT - FIELD_HEIGHT) / 2);
	...
	public void render(DrawContext context, TextRenderer textRenderer, int mouseX, int mouseY, float delta) {
		...
		if (label != null) {
			final String text = trim(textRenderer, label.getString(), textField.getX() - TEXT_INSET - LABEL_GAP - x);
			context.drawTextWithShadow(textRenderer, text, x + TEXT_INSET, y + (HEIGHT - 8) / 2, 0xFFFFFFFF);
		}
