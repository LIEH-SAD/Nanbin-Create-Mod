...
public class BlockBlueFence extends Block {
    ...
    public BlockState getPlacementState(ItemPlacementContext context) {
        ...
        if (canPlace) {
            ...
            world.setBlockState(clickPos.up(3), Blocks.BLUEFENCE_TOP.getDefaultState().with(FACING, facing), 3);
        ...
        while (true) {
            if (world.getBlockState(current).getBlock() instanceof BlockBlueFence || world.getBlockState(current).getBlock() instanceof BlockBlueFenceTop) {
                ...
                if (!player.isCreative()) {
                    dropStack(world, current, new net.minecraft.item.ItemStack(Blocks.BLUEFENCE));
                }
            }
