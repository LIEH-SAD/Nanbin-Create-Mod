...
public final class RoundMapDrawing {
	 ...
	 * @param font           文字字体（大小由调用方决定）
	 * @param routeNumber    线路编号，可为空
	 * @param platformNumber 站台号，可为空
	 */
	public static void draw(Graphics2D g, int centerX, int centerY, int radius, int strokeWidth, Color themeColor, Color textColor, Font font, String routeNumber, String platformNumber) {
		draw(g, centerX, centerY, radius, strokeWidth, themeColor, textColor, font, routeNumber, platformNumber, false);
	}
	 ...
	 */
	public static void drawInverted(Graphics2D g, int centerX, int centerY, int radius, int strokeWidth, Color themeColor, Color textColor, Font font, String routeNumber, String platformNumber) {
		draw(g, centerX, centerY, radius, strokeWidth, themeColor, textColor, font, routeNumber, platformNumber, true);
	}

	private static void draw(Graphics2D g, int centerX, int centerY, int radius, int strokeWidth, Color themeColor, Color textColor, Font font, String routeNumber, String platformNumber, boolean inverted) {
		...
		final int textLeading = Math.max(Math.round(font.getSize() * 0.15F), 2);
		...
		if (hasRoute) {
			final int rnW = metrics.stringWidth(routeNumber);
			g.setColor(drawTextColor);
			g.drawString(routeNumber, centerX - rnW / 2, cursorY + ascent);
			cursorY += ascent + descent + textLeading;
		}
		if (hasPlat) {
			final int pnW = metrics.stringWidth(platformNumber);
			g.setColor(drawTextColor);
			g.drawString(platformNumber, centerX - pnW / 2, cursorY + ascent);
		}
	}
}
