...
public class RenderCRTAPGDoor2<T extends BlockCRTAPGDoor2.BlockEntityBase> extends BlockEntityRenderer<T> implements IGui, IBlock {
    ...
    /** 屏蔽门编号铭牌：线路色正方形 + 白色编号，位于右门页平台侧外立面右侧、中上部。
     *  ModelSingleCube 的 z 经 z-8 映射，z=2 → 本地 z∈[-6,-5]，即贴住门页背板(z=-6)并向平台侧凸出。
     *  length/height=3 → 3/16 格小方块；depth=0.1 → 1/10 格厚。 */
    private static final ModelSingleCube CRT_APG_DOOR_NUMBER_2 = new ModelSingleCube(16, 16, 10, 4, 2, 3, 3, 0.1F);
    ...
    /** 门编号结果缓存时长：编号沿墙扫描开销较大（最多 512×2 格），避免每帧全量扫描。 */
    private static final long DOOR_NUMBER_CACHE_TTL_MS = 1000;

    private final Map<BlockPos, Double> previousOpenMap = new HashMap<>();
    /** doorPos -> {number, cacheTimeMillis}。 */
    private final Map<BlockPos, long[]> doorNumberCache = new HashMap<>();
    /** doorPos -> {lineColor, cacheTimeMillis}。 */
    private final Map<BlockPos, long[]> doorColorCache = new HashMap<>();
    ...
    private int getDoorLineColor(BlockPos doorPos) {
        try {
            ...
            InitClient.LOGGER.error("[CRTAPGDoor2] failed to resolve door line color", e);
        }
        return 0xFF929498;
    }

    /** 带 TTL 缓存的门编号：避免每帧沿墙全量扫描（最多 512×2 格），最多每秒重算一次。 */
    private int getDoorNumberCached(World world, BlockPos blockPos) {
        final long now = System.currentTimeMillis();
        final long[] cached = doorNumberCache.get(blockPos);
        if (cached != null && now - cached[1] < DOOR_NUMBER_CACHE_TTL_MS) {
            return (int) cached[0];
        }
        final int number = APGDoorNumbering.getDoorNumber(world, blockPos, IBlock.getStatePropertySafe(world, blockPos, BlockCRTAPGDoor2.SORT_FROM_RIGHT));
        doorNumberCache.put(blockPos, new long[]{number, now});
        if (doorNumberCache.size() > 512) {
            doorNumberCache.entrySet().removeIf(entry -> now - entry.getValue()[1] >= DOOR_NUMBER_CACHE_TTL_MS);
        }
        return number;
    }

    /** 带 TTL 缓存的线路色：避免每帧遍历全部站台与线路。 */
    private int getDoorLineColorCached(BlockPos doorPos) {
        final long now = System.currentTimeMillis();
        final long[] cached = doorColorCache.get(doorPos);
        if (cached != null && now - cached[1] < DOOR_NUMBER_CACHE_TTL_MS) {
            return (int) cached[0];
        }
        final int color = getDoorLineColor(doorPos);
        doorColorCache.put(doorPos, new long[]{color, now});
        if (doorColorCache.size() > 512) {
            doorColorCache.entrySet().removeIf(entry -> now - entry.getValue()[1] >= DOOR_NUMBER_CACHE_TTL_MS);
        }
        return color;
    }

    /** 在右门页外立面渲染屏蔽门编号铭牌（随门页开合移动）。 */
    private void renderDoorNumberPlate(World world, BlockPos blockPos, StoredMatrixTransformations storedMatrixTransformations, int light, int overlay) {
        final int doorNumber = getDoorNumberCached(world, blockPos);
        if (doorNumber > 0) {
            final int plateColor = getDoorLineColorCached(blockPos);
            final Identifier plateTexture = CustomFontTextureCache.instance.getDoorNumberPlateTexture(doorNumber, plateColor);
            /*
            MainRenderer.scheduleRender(plateTexture, false, QueuedRenderLayer.EXTERIOR, (graphicsHolder, offset) -> {
                storedMatrixTransformations.transform(graphicsHolder, offset);
                CRT_APG_DOOR_NUMBER_2.render(graphicsHolder, light, overlay, 1.0F, 1.0F, 1.0F, 1.0F);
                graphicsHolder.pop();
            });*/
        }
    }
        ...
        if (world != null) {
            ...
            switch (this.type) {
                ...
                case 2:
                    ...
                    if (!half && side) {
                        renderDoorNumberPlate(world, blockPos, storedMatrixTransformations, light, overlay);
                    }
                    if (half && !unlocked) {
    ...
    private static class ModelSingleCube extends EntityModelExtension<EntityAbstractMapping> {
        ...
        private ModelSingleCube(int textureWidth, int textureHeight, int x, int y, int z, float length, float height, float depth) {
            super(textureWidth, textureHeight);
            this.cube.setTextureUVOffset(0, 0).addCuboid((float)(x - 8), (float)(y - 16), (float)(z - 8), (int) length, (int) height, (int) depth, 0.0F, false);
            this.buildModel();
        }
