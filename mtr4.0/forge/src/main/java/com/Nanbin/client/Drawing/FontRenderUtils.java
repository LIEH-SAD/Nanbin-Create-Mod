...
public final class FontRenderUtils {
	...
	/**
	 * True if the text is a pure number: only ASCII digits, optionally separated by "/"
	 * (used when multiple route numbers are joined into one line). Used to decide whether
	 * SOURCE_SANS_3 (which has no CJK glyphs) is suitable for rendering.
	 */
