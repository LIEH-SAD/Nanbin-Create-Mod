...
public class RoadNameScreen extends MTRScreenBase implements IGui {
	...
	public RoadNameScreen(BlockPos blockPos, String[] initialTexts) {
		...
		this.currentTexts = this.initialTexts.clone();
	...
	protected void init2() {
		...
		for (int i = 0; i < textFields.length; i++) {
			...
			textFields[i].setText2(i < currentTexts.length ? currentTexts[i] : "");
			addChild(new ClickableWidget(textFields[i]));
			fieldY += ROW_SPACING;
		}

		// 保存按钮：把四个文本框内容发回服务器并关闭
		final ButtonWidgetExtension saveButton = new ButtonWidgetExtension(fieldX, fieldY + 6, fieldWidth , TEXT_FIELD_HEIGHT, TextHelper.translatable("gui.nanbin.save"), button -> {
		...
		final ButtonWidgetExtension undoButton = new ButtonWidgetExtension(buttonX, buttonY + 90, 30, 20, TextHelper.translatable("gui.nanbin.undo"), button -> {
			final String[] texts = getTexts();
			RoadNameClipboard.undo(blockPos, texts);
			setTexts(texts);
			button.setActiveMapped(RoadNameClipboard.canUndo(blockPos));
		});
		undoButton.active = RoadNameClipboard.canUndo(blockPos);

		final ButtonWidgetExtension copyButton = new ButtonWidgetExtension(buttonX, buttonY, 30, 20, TextHelper.translatable("gui.nanbin.copy"), button -> RoadNameClipboard.copy(getTexts()));

		final ButtonWidgetExtension pasteButton = new ButtonWidgetExtension(buttonX, buttonY + 30, 30, 20, TextHelper.translatable("gui.nanbin.paste"), button -> {
			final String[] texts = getTexts();
			RoadNameClipboard.paste(blockPos, texts);
			setTexts(texts);
			undoButton.setActiveMapped(RoadNameClipboard.canUndo(blockPos));
		});
		pasteButton.active = RoadNameClipboard.canPaste();

		final ButtonWidgetExtension clearButton = new ButtonWidgetExtension(buttonX, buttonY + 60, 30, 20, TextHelper.translatable("gui.nanbin.clear"), button -> MinecraftClient.getInstance().openScreen(new Screen(new RailwaySignClearConfirmScreen((ScreenExtension) (Object) this, TextHelper.translatable("gui.nanbin.clear.question"), () -> {
			final String[] texts = getTexts();
	...
	private void setTexts(String[] texts) {
		...
		for (int i = 0; i < Math.min(textFields.length, currentTexts.length); i++) {
			textFields[i].setText2(currentTexts[i]);
		}
	}
	...
	public void render(GraphicsHolder graphicsHolder, int mouseX, int mouseY, float delta) {
		try {
			...
			InitClient.LOGGER.error("Failed to render RoadNameScreen", e);
		}
	}
